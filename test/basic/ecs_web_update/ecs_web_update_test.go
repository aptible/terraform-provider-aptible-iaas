...
func TestECSWebPublicImageToPrivate(t *testing.T) {
	...
	terraformRegistrySecretOptions := terraform.WithDefaultRetryableErrors(t, &terraform.Options{
		...
		Vars: map[string]interface{}{
			...
			"environment_id":       os.Getenv("ENVIRONMENT_ID"),
			"aptible_host":         os.Getenv("APTIBLE_HOST"),
			"registry_secret_name": "registry1",
			"registry_secret_value": map[string]string{
	...
	defer cleanupAndAssert(t, terraformUpdateOptions)
	...
	ecsUpdateWebId := terraform.Output(t, terraformOptions, "ecs_web_id")
	assert.Equal(t, ecsUpdateWebId, ecsWebId)
	ecsUpdateWebAsset, ecsUpdateClusterAws, ecsUpdateServiceAws, ecsUpdateErr := getAptibleAndAWSECSServiceAndCluster(t, ctx, c, ecsWebId, "ecs-pub-web-test-web-cluster", "ecs-pub-web-test")
	...
	ecsUpdateLoadBalancerUrl := terraform.Output(t, terraformOptions, "loadbalancer_url")
	...
	ecsUpdateServiceUrl := terraform.Output(t, terraformOptions, "web_url")
